...
## File structure
```
pipeline/analysis/analyst.py       # retrieve_related_context(), analyze_change() — Grok call
pipeline/analysis/validator.py     # validate_signal() — embedding-similarity grounding check
```
...
4. Failed validation doesn't silently drop the signal — it's logged as a warning and the `signals.validated` column is set to `false`, so failure rate stays visible (see `pipeline/main.py`'s `_process_source()`).
