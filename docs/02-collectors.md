...
## File structure
```
pipeline/collectors/base.py               # BaseCollector: hashing, normalization, robots.txt check, rate limit, retry/backoff
pipeline/collectors/pricing_collector.py  # requests + BeautifulSoup
pipeline/collectors/feature_collector.py  # requests + BeautifulSoup
pipeline/collectors/jobs_collector.py     # Playwright (job boards are commonly JS-rendered)
pipeline/collectors/news_collector.py     # Hacker News Algolia search API (free, keyless)
pipeline/collectors/__init__.py           # COLLECTOR_BY_SOURCE_TYPE registry
```
...
4. Wire up real source rows in the `sources` table once competitor URLs are known — `pipeline/main.py`'s `_process_source()` picks the right collector via `COLLECTOR_BY_SOURCE_TYPE[source.source_type]`.
