...
infra/cloud_function/requirements.txt    # functions-framework + pipeline/requirements.txt
infra/scheduler_config.yaml              # Cloud Scheduler job definition (cron + target)
infra/sql/schema.sql                     # see 01-database.md
```

## Build steps
1. Deploy `infra/cloud_function/` as a GCP Cloud Function (HTTP-triggered), with `pipeline/` available as a dependency and `XAI_API_KEY`/`DATABASE_URL` set as function environment variables/secrets.
